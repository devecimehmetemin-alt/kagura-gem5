...
the way down. Compression helps in two ways there: the cache holds more, and
there is less dirty data for the checkpoint to write to non-volatile memory.
But compression costs energy, and the capacitor pays for it. Kagura's claim is
that compression should be turned off near the end of a power cycle: a block
compressed right before a failure dies with the SRAM and never pays back its
cost.

To test that, gem5 needs three things it does not have: a capacitor with a
power-failure model, a compressor that costs joules and not just cycles, and
the two compression policies. All of that is in `src/ehs/`.
...
pipelined CPU at an arbitrary moment; in-flight instructions panic at commit.
So a power failure is a handshake with the config script:
...
**NVM write energy.** Table I gives SRAM access and BDI compress/decompress
energies but no per-byte ReRAM write energy; the paper derives its figures
from McPAT/CACTI at 45 nm. The value here is a placeholder. Every
checkpoint-energy number is linear in it, which is why it gets swept instead
of fixed.

**Sampling.** `sample_interval` is not part of the published ACC. See the
`ACC` section above and `mSAMP.sh`.

---

## Where the results are

This document covers the code. The measurements, the figures, and the argument
they support are in the EUROP report. `sweep/results.csv` and the two report
scripts are what that report is built from.
