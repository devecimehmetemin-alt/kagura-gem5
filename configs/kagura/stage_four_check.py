...
# Stage 4 config
#  ACC's Global Compression Predictor gates each cache's compressor.
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Stage 4 run: intermittent power + Table I ReRAM + "
        "ACC-gated BDI compression (MinorCPU + SRAM L1s + checkpoint and "
    ...
    parser.add_argument(
        "--compression",
        choices=["none", "bdi", "acc"],
        default="acc",
        help="L1 cache compression: 'acc' (default) is BDI gated per cache by "
        "ACC's Global Compression Predictor; 'bdi' is the same compressor "
        "always on (the stage 3 result ACC must beat); 'none' is the plain "
        "uncompressed cache. Everything else is held fixed across the three.",
    )
...
CKPT_REG_BYTES = 68 + (4 if args.compression == "acc" else 0)
