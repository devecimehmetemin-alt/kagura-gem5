...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Stage 5 run: intermittent power + Table I ReRAM + "
        "Kagura mode switching over ACC-gated BDI compression (MinorCPU + "
        "SRAM L1s + checkpoint and compression energy accounting).",
    )
    parser.add_argument(
        "--cmd",
    ...
    parser.add_argument(
        ...
        "is the plain uncompressed cache. Everything else is held fixed.",
    )
...
# ACC reward cycles
REWARD_CYCLES_DCACHE = 17
REWARD_CYCLES_ICACHE = 19
...
system.cpu.icache = _l1_cache(REWARD_CYCLES_ICACHE)
system.cpu.dcache = _l1_cache(REWARD_CYCLES_DCACHE)
...
CKPT_REG_BYTES = (
    68
    + (4 if args.compression in ("acc", "kagura") else 0)
...
# commit-probe listeners, and the mode broadcast. Defaults for its knobs
# (n_thres_init=8, closeness_frac=0.1, sat_counter_init=1) live in
# KaguraController.py; n_thres_init is the paper's example value, the other
# two are choices the paper leaves open (flagged there).
_CONTROLLER = (
